...
import com.wassallni.data.datasource.ReservationDataSource
...
class ReservationRepository @Inject constructor(private val reservationDataSource: ReservationDataSource) :TripRepository(){
    ...
    fun callDistanceMatrixApi(userLocation: Origin, stations: List<Station>,mode:String): List<DistanceItem> {
        ...
        return reservationDataSource.callDistanceMatrixApi(origin, destinations,mode)
    }
    ...
    suspend fun bookTrip(fullTrip: FullTrip, point :Int,numOfSeat:Int):Boolean {
        val body=prepareBody(fullTrip,point,numOfSeat)
        val token=sharedPreferences.getString("token","")!!

        return reservationDataSource.bookTrip(token,body)
    }
    ...
    fun callGeocodeApi(latLng: LatLng?) :String {
        ...
        val respone=reservationDataSource.callGeocodeApi(latLngStr)
        val address=handlePolyLineResponse(respone)
        return address
    }
    ...
    fun chooseMovementMode(origin:LatLng, stations:List<Station>):String{
        ...
        var shortestDistance:Double=haversine(lat1,lon1,stations[0].location.lat,stations[0].location.lng)
